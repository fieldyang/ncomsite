...
export default class FormEx4 extends NComponent {
    ...
    return `
      <div>
        <nc-form label-width="100" label-align="right" dataSource={this.model} colon star gap-row="10">
          <nc-form-item label="用户名" name="username" required validator={{minLength,20]}}>
  ...
  checkPassword() {
    const pwd = this.root?.querySelector("[name='password']");
    const confirm = this.root?.querySelector("[name='confirmPassword']");
