...
export default class DocConcept extends NComponent {
  constructor() {
    super();
    this.addCss(css);
  }
  ...
  goExpression() {
    navigate("/doc/develop/expression");
  }
...
capture() {
  ...
  getSlotFlow() {
    return [
      { title: "parent 创建", content: "父组件实例化" },
      { title: "parent.constructor()", content: "解析 template，获取 root" },
      { title: "parent.attributeChangedCallback", content: "处理父组件属性" },
      { title: "parent.connectedCallback()", content: "父组件挂载到 DOM" },
      { title: "child.constructor()", content: "子组件开始创建" },
      { title: "child.attributeChangedCallback", content: "处理子组件属性" },
      { title: "child.connectedCallback()", content: "子组件挂载，插槽内容生效" },
    ];
  }
}

defineComponent("nc-doc-conceipt", DocConcept);
